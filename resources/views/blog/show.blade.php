...
<div class="container m-auto pt-10 pb-5 ">
    <div class="flex  ">
        <img class="object-cover h-80 w-full" src="/images/{{$post -> image_path}}" alt="">
    </div> 
    ...
    @if (auth::user() && auth::user()->id == $post->user_id)
         <a href="/blog/{{$post -> slug}}/edit" class="bg-green-700 text-gray-100 py-4 px-5 mt-6 inline-block rounded-lg font-bold uppercase text-l place-self-start">Edit Post</a>
         <form action="/blog/{{$post -> slug}}" method="post" class='inline-block'>
            @csrf
            @method('delete')
             <button  class="bg-red-700 text-gray-100 py-4 px-5 mt-6 inline-block rounded-lg font-bold uppercase text-l place-self-start">Delete Post</button>
         </form>
    @endif
